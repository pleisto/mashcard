...
module Docs
  module Queries
    class ChildrenBlocks < BrickGraphQL::BaseResolver
      ...
      def resolve(root_id:, snapshot_version:)
        ...
        if snapshot_version.zero?
          ...
          Docs::Snapshot.find_by!(block_id: root_id,
            snapshot_version: snapshot_version).blocks.graphql_normalize(root_id)
        end
      end
    end
  end
