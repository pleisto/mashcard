  ...
  module Queries
    class PageBlocks < BrickGraphQL::BaseResolver
      ...
      argument :domain, GraphQL::Types::String, required: true,
        description: 'List all pages for space domain'

      def resolve(domain:)
        blocks = Docs::Block.non_deleted.joins(:space).pageable.where(space: { domain: domain }).includes(:enabled_share_links).to_a
