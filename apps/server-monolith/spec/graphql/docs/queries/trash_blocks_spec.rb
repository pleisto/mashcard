...
describe Docs::Queries::TrashBlocks, type: :query do
  describe '#resolver' do
    ...
    it 'global' do
      ...
      internal_graphql_execute(query, { domain: pod.domain })
      expect(response.success?).to be true
      expect(response.data['trashBlocks'].any? { |b| b['id'] == block.id }).to be(true)

      internal_graphql_execute(query, { domain: pod.domain, search: 'baz' })
      expect(response.success?).to be true
      expect(response.data['trashBlocks']).to eq([])

      internal_graphql_execute(query, { domain: pod.domain, search: 'bar' })
      expect(response.success?).to be true
      expect(response.data['trashBlocks'].any? { |b| b['id'] == block.id }).to be(true)
    ...
    it 'current page (child page)' do
      ...
      internal_graphql_execute(query, { domain: pod.domain, blockId: root.id })
