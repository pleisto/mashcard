...
```js
const nodeModules = require('@brickdoc/webpacker/rules/node_modules.js')
...
const { environment, config } = require('@brickdoc/webpacker')
...
```js
const { environment } = require('@brickdoc/webpacker')
...
yarn upgrade @brickdoc/webpacker
```
...
- The majority of this config moved to the [@brickdoc/webpacker npm package](https://www.npmjs.com/package/@brickdoc/webpacker). `webpacker:install` only creates `config/webpack/{environment,development,test,production}.js` now so if you're upgrading from a previous version you can remove all other files.
