...
function collectSortStates<RecordType>(columns: ColumnsType<RecordType>, init: boolean, pos?: string): Array<SortState<RecordType>> {
  ...
  ;(columns || []).forEach((column, index) => {
    ...
    if ((column as ColumnGroupType<RecordType>).children) {
      ...
      sortStates = [...sortStates, ...collectSortStates((column as ColumnGroupType<RecordType>).children, init, columnPos)]
        ...
        title: (renderProps: ColumnTitleProps<RecordType>) => {
          const renderSortTitle = (
            <div className={`${prefixCls}-column-sorters`}>
              <span className={`${prefixCls}-column-title`}>{renderColumnTitle(column.title, renderProps)}</span>
              <span
...
export default function useFilterSorter<RecordType>({
  ...
  const [sortStates, setSortStates] = React.useState<Array<SortState<RecordType>>>(collectSortStates(mergedColumns, true))
  ...
  const transformColumns = (innerColumns: ColumnsType<RecordType>) =>
    injectSorter(prefixCls, innerColumns, mergedSorterStates, triggerSorter, sortDirections, tableLocale, showSorterTooltip)
