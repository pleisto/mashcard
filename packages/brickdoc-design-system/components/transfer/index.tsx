...
> {
  ...
  onItemSelectAll = (direction: TransferDirection, selectedKeys: string[], checkAll: boolean) => {
    this.setStateKeys(direction, prevKeys => {
      ...
      if (checkAll) {
        ...
        mergedCheckedKeys = prevKeys.filter((key: string) => !selectedKeys.includes(key))
      }
  ...
  separateDataSource() {
    ...
    const leftDataSource: Array<KeyWise<RecordType>> = []
    const rightDataSource: Array<KeyWise<RecordType>> = new Array(targetKeys.length)
