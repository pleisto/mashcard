...
export const FilterItem: React.FC<FilterItemProps> = ({ path, columns, filterSingleOption, onUpdateFilter }) => {
  ...
  const matchedColumnType = React.useMemo(
    () => COLUMN_TYPE.find(item => item.type === (matchedColumn as any)?.columnType),
    [matchedColumn]
  )
  ...
  const handleUpdateEventValue = (event: React.ChangeEvent<HTMLInputElement>): void =>
    onUpdateFilter({ value: event.target.value }, path)
  ...
  return (
    <>
      ...
      {isValueVisible('text') && (
        <TextValue onChange={handleUpdateEventValue} value={filterSingleOption.value as string} />
      )}
