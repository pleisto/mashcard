...
export const MentionCommandsMenu: React.FC<MentionCommandsMenuProps> = ({
  ...
  return (
    <div role="menu" className="brickdoc-mention-menu">
      <PeoplePanel active={activeCategory === 'people'} activeIndex={activeIndex} editor={editor} range={range} items={items.people} />
      <div className="mention-menu-divider" />
      <PagePanel active={activeCategory === 'page'} activeIndex={activeIndex} editor={editor} range={range} items={items.page} />
    </div>
  )
}
