...
describe('useList', () => {
  ...
  const setUp = (props: any): any => renderHook(() => useList(props))
  ...
  it('methods should work', () => {
    ...
    // addList
    act(() => {
      hook.result.current.addList([
        { name: 'ddd', age: 21 },
        { name: 'fff', age: 22 },
        { name: 'ggg', age: 23 }
      ])
    })
    expect(hook.result.current.list.length).toEqual(10)
  })
  ...
  it('initialValue changes', () => {
    const hook = renderHook(({ initialValue }) => useList(initialValue), {
      initialProps: {
        initialValue: [1]
      }
    })
