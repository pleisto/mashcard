...
pub fn aead_encrypt(plain: Either<String, Buffer>, key: String) -> Result<String> {
  ...
  xchacha20_cipher(key)?
    ...
    .map(|c| {
      let cipher = base64::url_safe_encode(c);
      let nonce = base64::url_safe_encode(nonce);
      format!("{}{}{}", nonce, B58_SAFE_SEPARATOR, cipher)
    })
}
...
pub fn aead_decrypt(cipher: String, key: String) -> Result<Buffer> {
  ...
  let nonce = base64::url_safe_decode(cipher_parts[0])
    .map_err(|e| Error::new(Status::InvalidArg, format!("Invalid nonce in input: {e}")))?;
  let cipher = base64::url_safe_decode(cipher_parts[1]).map_err(|e| {
    Error::new(
      Status::InvalidArg,
