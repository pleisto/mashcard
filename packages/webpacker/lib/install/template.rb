...
Dir.chdir(Rails.root) do
  if Webpacker::VERSION.match?(/^[0-9]+\.[0-9]+\.[0-9]+$/)
    ...
    results << run("yarn add @brickdoc/webpacker@#{Webpacker::VERSION}")
  else
    say "Installing all JavaScript dependencies [from prerelease rails/webpacker]"
    results << run("yarn add @brickdoc/webpacker@next")
  end
...
insert_into_file Rails.root.join("package.json").to_s, before: /\n}\n*$/ do
  <<~JSON.chomp
    ,
      "babel": {
        "presets": ["./node_modules/@brickdoc/webpacker/package/babel/preset.js"]
      },
      "browserslist": [
        "defaults"
      ]
