...
RSpec.describe Space, type: :model do
  context 'personal space' do
    it 'should personal space uniqueness' do
      user = create(:accounts_user)
      domain = Time.now.to_i.to_s(36)
      space = Space.new(owner: user, domain: domain, name: domain, personal: true)
    ...
    it '(personal space) member' do
      ...
      expect(user.spaces.find_by(id: space_id)).not_to eq(nil)
      expect(user.members.find_by(space_id: space_id)).not_to eq(nil)
    ...
    it 'space member' do
      ...
      expect(user2.spaces.find_by(id: space.id)).not_to eq(nil)
      expect(space.users.count).to eq(2)

      space.members.find_by!(user_id: user2.id).disabled!

      expect(user2.spaces.find_by(id: space.id)).to eq(nil)
      expect(space.users.count).to eq(1)
    end
  end
end
