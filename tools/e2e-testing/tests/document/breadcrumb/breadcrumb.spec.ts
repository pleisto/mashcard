...
test.describe('Breadcrumb', () => {
  ...
  test('Verify the initial breadcrumbs count equal 1 and named Untitled', async ({ api }) => {
    ...
    await expect(breadcrumb.items).toHaveCount(1)
    await expect(breadcrumb.texts.nth(0)).toHaveText(INITIAL_PAGE.title)
  })
  ...
  test('Verify breadcrumbs count equal 2 when add a sub page', async ({ api }) => {
    ...
    await expect(breadcrumb.items).toHaveCount(2)
  })
  ...
  test('Verify breadcrumb width is less than 400px', async ({ api }) => {
    ...
    await expect(breadcrumb.items.nth(0)).toHaveCSS('width', lessThan400px)
  })
  ...
  test('Verify breadcrumb max width is equal 400px', async ({ api }) => {
    ...
    await expect(breadcrumb.items.nth(0)).toHaveCSS('width', '400px')
    await expect(breadcrumb.texts.nth(0)).toHaveCSS('overflow', 'hidden')
    await expect(breadcrumb.texts.nth(0)).toHaveCSS('text-overflow', 'ellipsis')
    await expect(breadcrumb.texts.nth(0)).toHaveCSS('white-space', 'nowrap')
  })
  ...
  test('Verify breadcrumb tooltip is working with icon', async ({ api }) => {
    ...
    await pageTree.clickPage()
    await breadcrumb.texts.nth(0).hover()

    await expect(breadcrumb.tooltip).toContainText(SUPER_LONG_TITLE_PAGE.icon?.emoji as string)
  })
  ...
  test('Verify breadcrumb tooltip is working without icon', async ({ api }) => {
    ...
    await expect(breadcrumb.tooltip).toContainText(SUPER_LONG_TITLE_PAGE.title)
  })
  ...
  test('Verify breadcrumb max layer equal 4', async ({ api }) => {
    ...
    await expect(breadcrumb.items).toHaveCount(4)
    await expect(breadcrumb.items.nth(1)).toContainText('...')
  })
  ...
  test('Verify the omitted item exists in the menubar when hover on ... breadcrumb', async ({ api }) => {
    ...
    await pageTree.expandSubPageOneByOne(4)
    await pageTree.clickPage(4)
    await expect(breadcrumb.texts.nth(2)).toHaveText('page 1-1-1-1-1')

    await breadcrumb.items.nth(1).hover()
    await expect(breadcrumb.menubarItems).toHaveCount(2)
  })
