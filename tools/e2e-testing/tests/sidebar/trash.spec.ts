...
import { TRASH_PAGE_TREE, TRASH_SINGLE_PAGE } from '@/data/trash.data'
...
test.describe('Trash', () => {
  ...
  test.describe('Trash Item', () => {
    ...
    test('Verify restore item is working well', async ({ page }) => {
      ...
      await expect(pageList.getPageByIndex()).toContainText(TRASH_SINGLE_PAGE.title)
    })
  ...
  test.describe('Selected Bar', () => {
    ...
    test('Verify restore selected is working well', async ({ page }) => {
      ...
      await expect(pageList.getPageByIndex()).toContainText(TRASH_PAGE_TREE[1].title)
      await expect(pageList.getPageByIndex(1)).toContainText(TRASH_PAGE_TREE[2].title)
    })
